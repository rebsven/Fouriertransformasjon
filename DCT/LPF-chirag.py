...
def transform(img, inp):
    ...
    mask[-int((var/10)*rows):, :] = not replaceValue
    mask[:int((var/10)*rows), :] = not replaceValue
    mask[:, -int((var/10)*cols):] = not replaceValue
    mask[:,:int((var/10)*cols)] = not replaceValue
    if var == 0:
        mask[:, :] = replaceValue
    ...
    masked_magnitude_spectrum[:int((var/10)*rows), :] = replaceValue
    masked_magnitude_spectrum[-int((var/10)*rows):, :] = replaceValue
    masked_magnitude_spectrum[:,:int((var/10)*cols)] = replaceValue
    masked_magnitude_spectrum[:, -int((var/10)*cols):] = replaceValue
    if var == 0:
...
img = cv2.imread('ChiragBilde.jpg')
...
for i in range(11):
    ...
    plt.savefig(f'moro2/chirag_{i}0_percent.jpg', format='jpeg', dpi=1200)     
    print(f"Lagret nr {i}")
